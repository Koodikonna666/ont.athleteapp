package ont.athleteapp.training.speed;

import ont.athleteapp.training.Training;

import javax.persistence.*;

@Entity
@Table(name = "speed_training")
public class SpeedTraining  {

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "speed_training_sequence")
    @SequenceGenerator(name = "speed_training_sequence", sequenceName = "speed_training_sequence")
    @Column(updatable = false)
    private Long id;
    private String blocks;
    private String subMax;
    private String maxSpeed;
    private String assistedSpeed;
    private String comments;

    @MapsId
    @OneToOne(cascade = CascadeType.ALL, mappedBy = "speedTraining")
    private Training training;

    public SpeedTraining() {

    }
    public SpeedTraining(String blocks, String subMax, String maxSpeed, String assistedSpeed, String comments){
        this.assistedSpeed = assistedSpeed;
        this.maxSpeed = maxSpeed;
        this.subMax = subMax;
        this.blocks = blocks;
        this.comments = comments;
    }

    public SpeedTraining(String blocks, String subMax, String maxSpeed, String assistedSpeed, String comments, Training training) {
        this.assistedSpeed = assistedSpeed;
        this.maxSpeed = maxSpeed;
        this.subMax = subMax;
        this.blocks = blocks;
        this.comments = comments;
        this.training = training;
    }

    public String getBlocks() {
        return blocks;
    }

    public void setBlocks(String blocks) {
        this.blocks = blocks;
    }

    public String getSubMax() {
        return subMax;
    }

    public void setSubMax(String subMax) {
        this.subMax = subMax;
    }

    public String getMaxSpeed() {
        return maxSpeed;
    }

    public void setMaxSpeed(String maxSpeed) {
        this.maxSpeed = maxSpeed;
    }

    public String getAssistedSpeed() {
        return assistedSpeed;
    }

    public void setAssistedSpeed(String assistedSpeed) {
        this.assistedSpeed = assistedSpeed;
    }

    public String getComments(){
        return comments;
    }
    public void setComments(String comments){
        this.comments = comments;
    }


    @Override
    public String toString() {
        return "SpeedTraining{" +
                "blocks='" + blocks + '\'' +
                ", subMax='" + subMax + '\'' +
                ", maxSpeed='" + maxSpeed + '\'' +
                ", assistedSpeed='" + assistedSpeed + '\'' +
                ", comments='" + comments + '\'' +
                '}';
    }
}
